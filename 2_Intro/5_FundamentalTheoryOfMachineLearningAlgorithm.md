...
## 6.贝叶斯统计
    ...
    参考:https://zhuanlan.zhihu.com/p/38553838
...
简而言之：通过已知的概率模型来精确的计算各种结果的可能性就是概率论；根据观测的结果来推断模型的不确定性就是统计学。

### 6.2 频率派和贝叶斯统计学派

统计学领域中有两大学派：古典统计学（classical，又称为频率论frequentist）和贝叶斯统计学（Bayesian，以英国数学家托马斯•贝叶斯命名）。它们的本质区别在于对待未知模型或者参的方法是不同的：

- 古典统计学认为，未知的模型或者参数是确定的，只不过我们不知道它确切的形式或者取值。
  - 通过进行大量重复实验并统计某个特定结果出现的频率作为对未知参数的估计。
- 贝叶斯统计学认为，未知的模型或者参数变量是不确定的，但是这种不确定性可以由一个概率分布来描述
  - 使用概率的方法来解决统计学问题
    - 先验分布(prior distribution):先根据主观判断或者过去的经验，对这个概率分布有一个猜测
    - 观测值(new data 或者 new evidence)：来修正对该概率分布的猜测
    - 后验分布(posterior distribution):最后得到的概率分布
...
- 生活中的很多实际问题，大量重复实验是不现实的
- 合理的先验分布对未知量的估计是非常有益的

### 6.4 贝叶斯推断

通过新证据或者数据来更新认知的过程就是贝叶斯推断。

假设我们有一个需要估计的未知量$\theta$ ，并且针对该变量有一个先验分布$P(\theta)$ 。令$D$为一系列观测值或者证据。我们希望通过$D$来修正对$\theta$的分布的认知，即$P(\theta|D)$。由贝叶斯定理可得：
...
贝叶斯推断框架的特点也是其强大之处在于：我们可以迭代的看问题，即在每次有新观测数据后我们可以得到一个新的后验分布，然后把它作为下个新数据出现前的（新的）先验分布。我们通过反复迭代使用贝叶斯定理，持续更新对未知量的分布的认知。
